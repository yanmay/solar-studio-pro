...
import { ArrowLeft, Download, Square, Zap, IndianRupee, Leaf } from "lucide-react";
...
const mockData = {
  ...
  dailyKwh: 49.9,
  monthlyKwh: 1519,
  monthlySavings: 10637,
  twentyFiveYearSavings: 3191050,
  co2_25yr: 373800,
  trees: 680,
};
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <MetricCard
            ...
            label="Roof Area"
            value={`${mockData.area} m²`}
            subLabel={`Usable: ${mockData.usable} m²`}
          ...
          <MetricCard
            ...
            value={`${mockData.kw} kW`}
            subLabel={`Annual: ${mockData.kwh.toLocaleString()} kWh`}
            valueColor="#E65100"
            delay={80}
          />
          <MetricCard
          ...
          <MetricCard
            ...
            iconBg="hsl(88 44% 91%)"
            label="CO₂ Saved"
            value={`${mockData.co2.toLocaleString()} kg`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="space-y-0">
                {[
                  { label: "Daily Average", value: `${mockData.dailyKwh} kWh` },
                  { label: "Monthly Average", value: `${mockData.monthlyKwh.toLocaleString()} kWh` },
                  { label: "Annual Total", value: `${mockData.kwh.toLocaleString()} kWh` },
                ].map((row, i, arr) => (
            ...
            <div>
              ...
              <div className="space-y-0">
                {[
                  { label: "Monthly Savings", value: `₹${mockData.monthlySavings.toLocaleString()}` },
                  { label: "Annual Savings", value: `₹${mockData.savings.toLocaleString()}` },
                  { label: "25-Year Savings", value: `₹${mockData.twentyFiveYearSavings.toLocaleString()}` },
                ].map((row, i, arr) => (
                  <div key={row.label} className={`flex justify-between items-center py-3 ${i < arr.length - 1 ? "border-b border-dashed border-foreground/[0.08]" : ""}`}>
                    <span className="text-sm text-urja-text-secondary">{row.label}</span>
        ...
        <div className="bg-urja-success-light border border-urja-success/20 rounded-lg p-6 sm:p-8">
          ...
          <p className="text-[15px] text-urja-text-secondary mb-6">
            ...
            <span className="font-semibold text-urja-success">{mockData.co2.toLocaleString()} kg</span> of CO₂ per year, equivalent to planting{" "}
            <span className="font-semibold text-urja-success">{mockData.trees}</span> trees.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <div className="text-sm text-urja-text-secondary mb-1">25-Year CO₂ Reduction</div>
              <div className="font-mono text-2xl font-semibold text-urja-success">
                {mockData.co2_25yr.toLocaleString()} kg
              </div>
            </div>
            <div>
              <div className="text-sm text-urja-text-secondary mb-1">Equivalent Trees</div>
              <div className="font-mono text-2xl font-semibold text-urja-success">
                {mockData.trees} trees
              </div>
            </div>
          </div>
        </div>
